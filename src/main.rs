...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let opt_columns: StringConfigOption = ConfigOption::new_str_no_default(
        ...
        `GRAPH_SATS,PERC_US,OUT_SATS,IN_SATS,TOTAL_SATS,SCID,MIN_HTLC,MAX_HTLC,FLAG,BASE,PPM,ALIAS,\
        PEER_ID,UPTIME,HTLCS,STATE` Default is `OUT_SATS,IN_SATS,SCID,MAX_HTLC,FLAG,BASE,PPM,ALIAS,\
        PEER_ID,UPTIME,HTLCS,STATE`",
    )
    .dynamic();
