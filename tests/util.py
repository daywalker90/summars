...
def get_plugin(directory):
    downloaded_plugin_path = Path(
        os.path.join(directory, "summars.tar.gz"))
    extracted_plugin_path = Path(os.path.join(directory, "summars"))
    if COMPILED_PATH.is_file():
        ...
        url = (f"https://github.com/daywalker90/summars/releases/download/"
               f"{VERSION}/summars-{VERSION}-{architecture}.tar.gz")
