...
github_url="https://github.com/daywalker90/summars/releases/download/v$version/summars-v$version-$architecture.tar.gz"


# Download the file using curl
if ! curl -L "$github_url" -o "$script_dir/summars-v$version-$architecture.tar.gz"; then
    echo "Error downloading the file from $github_url" >&2
    exit 1
fi

# Extract the contents using tar
if ! tar -xzvf "$script_dir/summars-v$version-$architecture.tar.gz" -C "$script_dir"; then
    echo "Error extracting the contents of summars-v$version-$architecture.tar.gz" >&2
    exit 1
fi